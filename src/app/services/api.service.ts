...
export class ApiService {
  ...
  readonly COMPETITION = 2019;

  constructor(private http: HttpClient) {
  }

  getTeams(start: number, rows: number): Observable<PaginationTeam> {
    return this.http.get<PaginationTeam>(`${this.baseUrl}/competitions/${this.COMPETITION}/teams`).pipe(map(pt => ({
      ...pt,
  ...
  getTeam(id: number): Observable<TeamView> {
    return this.http.get<TeamDetail>(`${this.baseUrl}/teams/${id}?season=2020`).pipe(map((team: TeamDetail) => ({
      ...team,
